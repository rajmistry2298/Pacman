...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        startStateNode = (startPosition, cornersList)
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        #If Length of State[1] == 0 means no more elements so it is Goal State, otherwise it is not goal state
        if len(state[1]) == 0:
            goalState = True
        else:
            goalState = False
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                for i in range(len(state[1])):
                    if not (nextx, nexty) == state[1][i]:
                        notVisited.append(state[1][i])
                remainingCorners = tuple(notVisited)
...
def cornersHeuristic(state, problem):
    ...
    for i in range(len(states)):
        if not states[i] == None:
            currentdistance = util.manhattanDistance(state[0], states[i])
            if currentdistance > maxHeuristicDistance:
                maxHeuristicDistance = currentdistance
    
    return maxHeuristicDistance # Default to trivial solution
